...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const [dbUrlInput, setDbUrlInput] = useState<string>(() => localStorage.getItem('cafe_pos_supabase_url') || '');
  const [dbKeyInput, setDbKeyInput] = useState<string>(() => localStorage.getItem('cafe_pos_supabase_key') || '');

  const handleSaveCloudDbKeys = () => {
    if (dbUrlInput.trim()) {
      localStorage.setItem('cafe_pos_supabase_url', dbUrlInput.trim());
    } else {
      localStorage.removeItem('cafe_pos_supabase_url');
    }

    if (dbKeyInput.trim()) {
      localStorage.setItem('cafe_pos_supabase_key', dbKeyInput.trim());
    } else {
      localStorage.removeItem('cafe_pos_supabase_key');
    }

    posSound.playSuccess();
    alert('Cloud DB keys saved! Reloading system to connect cloud database.');
